...
namespace DbxUtils.Utils
{
    ...
    {
        public static ObjectId AddNewWhiteLayer(Database database, string layerName, short layerColor)
        {
            var result = ObjectId.Null;
            if (HasLayer(database, layerName))
            {
                result = UpdateLayerColor(database, layerName, layerColor);
            }
            else
            {
                result = AddNewLayer(database, layerName, "Continuous", layerColor);
            }
            return result;
        }
        ...
        public static ObjectId UpdateLayerColor(Database database, string layerName, short layerColor)
        {
            var result = ObjectId.Null;
        ...
        public static bool IsAnyEntityOnLockedLayer(IEnumerable<ObjectId> entities, out string layerName)
        {
            ...
            using (var tran = entities.First().Database.TransactionManager.StartTransaction())
            {
                foreach (var objectId in entities)
                {
                    ...
                    {
                        layerName = existedLayerTableRecord.Name;
                        return true;
                    }
                }
            }
        ...
        public static void SetLayerOff(Database database, string layerName, bool off)
        {
            if (database == null) 
                throw new ArgumentNullException(/*MSG0*/"database");
