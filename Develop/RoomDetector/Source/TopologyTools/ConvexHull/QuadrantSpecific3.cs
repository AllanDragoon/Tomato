...
{
	public class QuadrantSpecific3<T> : Quadrant<T>
	{
		// ************************************************************************
        public QuadrantSpecific3(IEnumerable<IEnumerable<T>> listOfListOfPoint, int initialResultGuessSize,
		...
		{
			if (_xGet(pt).SmallerWithTol( _xGet(this.RootPoint) )&& _yGet(pt).SmallerWithTol( _yGet(this.RootPoint)))
			{
				return true;
			}
		...
		protected override void TryAdd(double x, double y)
		{
			...
			if (y.LargerOrEqualWithTol( ptiBefore.Y))
			{
			...
			if (slopeToNext.LargerWithTol( ptiBefore.SlopeToNext))
			{
				...
				if (HullPoints[indexHi].Y.LargerWithTol( y)) // We can remove 1 or more points without slope calc
				{
					indexHiToRemove = indexHi;
					while (HullPoints[indexHiToRemove + 1].Y.LargerWithTol( y))
					{
						indexHiToRemove++;
					}
				}
